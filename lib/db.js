...
export async function migrate() {
  await query(`
    ...
    /* SEO + Google metrics are stored as daily snapshots so the dashboard can
       render historical charts even when Google APIs are queried once per day. */
    CREATE TABLE IF NOT EXISTS seo_daily_metrics (
      ...
      gsc_position             REAL NOT NULL DEFAULT 0,
      gsc_top_queries          JSONB NOT NULL DEFAULT '[]'::jsonb,
      gsc_top_pages            JSONB NOT NULL DEFAULT '[]'::jsonb,
      ga_users                 INTEGER NOT NULL DEFAULT 0,
      ga_sessions              INTEGER NOT NULL DEFAULT 0,
      ga_pageviews             INTEGER NOT NULL DEFAULT 0,
      ga_engagement_rate       REAL NOT NULL DEFAULT 0,
      ga_avg_engagement_seconds REAL NOT NULL DEFAULT 0,
      ga_conversions           INTEGER NOT NULL DEFAULT 0,
      blog_views               INTEGER NOT NULL DEFAULT 0,
      lead_submissions         INTEGER NOT NULL DEFAULT 0,
      notes                    JSONB NOT NULL DEFAULT '{}'::jsonb,
      updated_at               TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    CREATE INDEX IF NOT EXISTS idx_seo_daily_updated ON seo_daily_metrics(updated_at DESC);
  `);

  /* Marketing attribution (UTM, referrer, landing page) for leads. Added after
     the leads table shipped, so ALTER rather than a column in CREATE TABLE. */
  await query(`
...
export async function createLead(data) {
  const { rows } = await query(
    `INSERT INTO leads (kind, name, email, phone, company, team_size, message, attribution)
...
export async function upsertSeoDailyMetrics(day, data) {
  const d = String(day).slice(0, 10);
  const payload = data || {};
  await query(
    `INSERT INTO seo_daily_metrics (
      day, gsc_impressions, gsc_clicks, gsc_ctr, gsc_position,
      gsc_top_queries, gsc_top_pages,
      ga_users, ga_sessions, ga_pageviews, ga_engagement_rate,
      ga_avg_engagement_seconds, ga_conversions,
      blog_views, lead_submissions, notes, updated_at
    ) VALUES (
      $1,$2,$3,$4,$5,
      $6::jsonb,$7::jsonb,
      $8,$9,$10,$11,
    ...
    ON CONFLICT (day) DO UPDATE SET
      ...
      gsc_top_pages = EXCLUDED.gsc_top_pages,
      ga_users = EXCLUDED.ga_users,
      ga_sessions = EXCLUDED.ga_sessions,
      ga_pageviews = EXCLUDED.ga_pageviews,
      ga_engagement_rate = EXCLUDED.ga_engagement_rate,
      ga_avg_engagement_seconds = EXCLUDED.ga_avg_engagement_seconds,
      ga_conversions = EXCLUDED.ga_conversions,
      blog_views = EXCLUDED.blog_views,
      lead_submissions = EXCLUDED.lead_submissions,
      notes = EXCLUDED.notes,
      updated_at = now()`,
    [
      d,
      Number(payload.gsc_impressions) || 0,
      Number(payload.gsc_clicks) || 0,
      Number(payload.gsc_ctr) || 0,
      Number(payload.gsc_position) || 0,
      JSON.stringify(Array.isArray(payload.gsc_top_queries) ? payload.gsc_top_queries : []),
      JSON.stringify(Array.isArray(payload.gsc_top_pages) ? payload.gsc_top_pages : []),
      Number(payload.ga_users) || 0,
      Number(payload.ga_sessions) || 0,
      Number(payload.ga_pageviews) || 0,
      Number(payload.ga_engagement_rate) || 0,
      Number(payload.ga_avg_engagement_seconds) || 0,
      Number(payload.ga_conversions) || 0,
      Number(payload.blog_views) || 0,
      Number(payload.lead_submissions) || 0,
      JSON.stringify(payload.notes || {}),
    ]
  );
}

export async function seoDashboardSnapshot(limit = 30) {
  const { rows } = await query(
    `SELECT * FROM seo_daily_metrics ORDER BY day DESC LIMIT $1`, [limit]
  );
  return rows;
}
