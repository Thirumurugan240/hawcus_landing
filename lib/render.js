...
${extraHead}
  ...
  fbq('init', '2849339562076470');
  fbq('track', 'PageView');
  </script>
  <noscript><img height="1" width="1" style="display:none"
  src="https://www.facebook.com/tr?id=2849339562076470&ev=PageView&noscript=1"
  /></noscript>
  <!-- End Meta Pixel Code -->
  <!-- Microsoft Clarity -->
  <script type="text/javascript">
    ...
    })(window, document, "clarity", "script", "xr14mewk0z");
  </script>
  <!-- End Microsoft Clarity -->
  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-Q2DKRY62K7"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());
    gtag('config', 'G-Q2DKRY62K7');
  </script>
  <!-- End Google tag -->
</head>
<body>
...
export function renderArticle(post, faqs, related) {
  ...
  /* The cover infographic's "what you will learn" checklist is built from the
     article's own top-level (H2) sections, so an author never has to write or
     upload anything extra. FAQ is excluded because it is added to the TOC below. */
  const coverPoints = toc
    .filter((t) => t.level === "2" && t.id !== "faq")
    .map((t) => t.text)
    .slice(0, 4);
...
${faqs.map((f) => `              <div class="afaq__item">
  ...
  const coverInner =
    coverPoints.length >= 2
      ? `              <b class="acover__lead">What you will learn</b>
              <ul class="acover__list">
${coverPoints.map((p) => `                <li>${check}<span>${esc(p)}</span></li>`).join("\n")}
              </ul>`
      : `              <p class="acover__sum">${esc(post.tldr || post.excerpt || "")}</p>`;

  const coverBlock = `            <div class="acover" aria-hidden="true">
              <div class="acover__info">
                <span class="acover__eyebrow">${iconFor(post.category)}${esc(post.category)}</span>
${coverInner}
                <div class="acover__chips">
                  <span class="acover__chip">${post.read_minutes} min read</span>
                  <span class="acover__chip">Updated ${fmtDate(post.updated_at || post.published_at)}</span>
                </div>
              </div>
              <div class="acover__art" style="background:${esc(gradFor(post.category, post.banner_grad))}">
                <span class="acover__b acover__b--1"></span>
                <span class="acover__b acover__b--2"></span>
                <span class="acover__b acover__b--3"></span>
                <span class="acover__glyph">${iconFor(post.category)}</span>
              </div>
            </div>`;
...
${faqBlock}
            ...
            <div class="abio">
              ...
              <div class="abio__body">
                ...
                <p>The Hawcus team builds a Sales Growth System for Business used by growing sales teams across India. We write about the things we see working in real pipelines, not theory.</p>
